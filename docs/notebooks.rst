...
As of the most recent headless run (``pytest --nbmake``, ~80 seconds total for all 45), **35 of 45 pass** in a network-isolated environment. The other 10 call ``rdr.download()`` or ``rdr.catalog(location='remote')`` against the real public library at distantreader.org and need outbound network access to pass; every one of their failures was traced to a network-layer error (``BadZipFile``, ``JSONDecodeError``) rather than a code bug, so they are **not known-broken**, just unverified in a network-isolated sandbox. None of the 45 notebooks are labeled *retire* -- every one that could be run headlessly against ``mini`` has been fixed to do so.
